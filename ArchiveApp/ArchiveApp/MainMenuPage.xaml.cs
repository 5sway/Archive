...
namespace ArchiveApp
{
    public partial class MainMenuPage : Page
    {
        ...
        /// <summary>
        /// Настраивает видимость и расположение кнопок в зависимости от роли пользователя.
        /// Администратор видит все разделы, делопроизводитель ограничен документами и карточками,
        /// архивариус получает доступ к запросам и документам.
        /// Также корректируются границы кнопок для сохранения целостности интерфейса.
        /// </summary>
        ...
        private void SimpleRepBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    HandleReportOptions(format, isTableFormat, tables, selectedRecordIds, startDate, endDate, "Простой отчет");
                };
        ...
        private void ReportBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                optionsPage.ReportOptionsSelected += (format, isTableFormat, tables, selectedRecordIds, startDate, endDate) =>
                {
                    HandleReportOptions(format, isTableFormat, tables, selectedRecordIds, startDate, endDate, "Отчет");
                };
                Manager.MainFrame.Navigate(optionsPage);
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Обрабатывает параметры отчёта после выбора пользователем.
        /// В зависимости от типа отчёта (полный или простой) корректирует диапазон дат,
        /// открывает диалог сохранения файла и инициирует экспорт.
        /// </summary>
        private void HandleReportOptions(string format, bool isTableFormat, List<string> tables,
            Dictionary<string, List<int>> selectedRecordIds, DateTime? startDate, DateTime? endDate, string defaultFileName)
        {
            try
            {
                ...
                bool isFullReport = defaultFileName != "Простой отчет";
                ...
                if (saveFileDialog.ShowDialog() == true)
                {
                    ...
                    ExportReport(filePath, format, isTableFormat, tables, selectedRecordIds, _Role, startDate, endDate);
        ...
        /// <summary>
        /// Выполняет экспорт отчёта в выбранном формате.
        /// В зависимости от расширения делегирует работу соответствующим сервисам.
        /// </summary>
        private void ExportReport(string filePath, string format, bool isTableFormat, List<string> tables, Dictionary<string, List<int>> selectedRecordIds, string role, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                switch (format.ToLower())
                {
                    case "word":
                    case "pdf":
                        ExportWord.ExportToWord(filePath, tables, selectedRecordIds, startDate, endDate, role, format, isTableFormat);
                        break;
                    case "excel":
                        ExportExcel.ExportToExcel(filePath, tables, selectedRecordIds, role, startDate, endDate);
                        break;
                    default:
